...
function autocompleteInputValues(element, strLenght, strChart, padRight) {
    padRight = padRight === true ? true : false;
    if (isNullorEmpty(element.value)) {
        if (!padRight)
            $(element).val($.lpad(element.value, strLenght, strChart));
        else
            $(element).val($.rpad(element.value, strLenght, strChart));
    }
}
...
(function ($) {
    ...
    $.fn.htmlSelectChangeName = function () {
        var newName = "";
        $("select", this).each(function () {
            newName = $(this).attr('data-changename');
            if (newName) {
...
(function ($) {
    var Dominios = {
        ...
        TipoRoca: function () {
            return { Opcion: "04_RH_TIPO" };
        },
        SubTipoRoca: function (value, union) {
            union = union || '|';
            return { Opcion: "04_RH_SUBTIPO", Descripcion: value + union }
        },
        Vertiente: function () {
